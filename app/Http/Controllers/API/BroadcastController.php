...
class BroadcastController extends Controller
{
    ...
    public function sendBroadcast(Request $request)
    {
        ...
        foreach ($users as $user) {
            $messages[] = [
                ...
                'message' => 'Halo ' . $user->nama . ', ini adalah pesan broadcast. ' . $request->message, // Pesan sebagai string
                'delay' => '1', // Jeda 1 detik untuk setiap pengiriman (gunakan string)
            ];
        }
    ...
    public function broadcastByGender(Request $request)
    {
        // Validasi input
        $request->validate([
        ...
        $gender = $request->input('gender'); // Gender yang diinputkan
        $message = $request->input('message'); // Pesan yang diinputkan



        // Ubah gender input menjadi format database (male -> pria, female -> wanita)


        // Ambil semua nomor WhatsApp pengguna berdasarkan gender
        ...
        foreach ($users as $user) {
            $messages[] = [
                'target' => (string) $user->no_whatshap, // Pastikan target dalam format string
                'message' => $message, // Pesan yang dikirimkan
                'delay' => '1', // Jeda 1 detik untuk setiap pengiriman
            ];
        }
        ...
        // return [$gender, $message, $users, $messages];
    }
    ...
    public function broadcastByAge(Request $request)
    {
        ...
        foreach ($users as $user) {
            $messages[] = [
                'target' => (string) $user->no_whatshap, // Pastikan target dalam format string
                'message' => $message . ' ' . $user->nama . ' pesan ini untuk umur antara ' . $awal . ' hingga ' . $akhir, // Pesan yang dikirimkan
                'delay' => '1', // Jeda 1 detik untuk setiap pengiriman
            ];
        }
